...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const board = await ctx.db.insert('boards', {
      ...
      authorizationId: identity.subject,
      authorizationName: identity.name!,
...
export const favorite = mutation({
  args: { id: v.id('boards'), organizationId: v.string() },
  handler: async (ctx, args) => {
    const identity = await ctx.auth.getUserIdentity()
    if (!identity) {
      throw new Error('Unauthorized')
    ...
    const existingFavorite = await ctx.db
      ...
      .withIndex('by_user_board_organization', (q) =>
        q.eq('userId', userId).eq('boardId', board._id).eq('organizationId', args.organizationId)
      )
      .unique()
